...
def get_current_time():
    ...
    return str(date_obj.hour) + ':' + str(date_obj.minute) + ':' + str(date_obj.second)
...
def on_new_client(client_socket, addr):
    try:
        with client_socket:
            ...
            print(f'Message: {data} from {addr} at {get_current_time()}')
            client_socket.send(bytes(get_current_time(), encoding='utf-8'))
